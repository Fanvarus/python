            ...
            for root, _, files in os.walk(self.source_folder):
                for file in files:
                    if file.lower().endswith(".png"):
                        ...
                        phone, wechat_name = (file_name.split("_", 1) if "_" in file_name else (file_name, "未命名"))
                        ...
                        self.log_signal.emit(f"已扫描：{file} | 手机号：{phone_clean} | 联系人：{contact}")
...
class AvatarSummaryWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("凡先生-头像汇总工具-V3.4")
    ...
    def init_log(self):
        self.log("欢迎使用头像汇总工具（科技版 V3.4 - 柔和配色版）")
        self.log("表格优化特性：")
